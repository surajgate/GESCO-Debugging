...
from db import get_db, chats, user_departments
...
NUM_CHUNKS_TO_SIMILARITY_SEARCH = int(
    os.getenv("NUM_CHUNKS_TO_SIMILARITY_SEARCH", default="100"))
DEPARTMENT_ID = os.getenv("DEPARTMENT_ID", "")
DENSE_VECTOR_WEIGHT = float(os.getenv("DENSE_VECTOR_WEIGHT", ""))
SPARSE_VECTOR_WEIGHT = float(os.getenv("SPARSE_VECTOR_WEIGHT", ""))
INSTANCE_TYPE = os.getenv("INSTANCE_TYPE", "CHATGPT")

if INSTANCE_TYPE == 'CHATGPT':
    OPENAI_EMBEDDINGS_MODEL = os.getenv("OPENAI_EMBEDDINGS_MODEL")
elif INSTANCE_TYPE == 'AZURE-CHATGPT':
...
def fetch_answer_without_citations():
    ...
    try:
        ...
        # 04:30 UTC (10 AM IST) case
        if last_report_time == REPORT_TIMES_UTC[0]:
            ...
            .filter(
                or_(
                    ...
                    func.jsonb_array_length(
                        func.cast(chats.c.citations, JSONB)) == 0
                )
            )
        ...
        print(
            f"An error occurred while fetching answers without citations: {e}")
        return None
    finally:
        db.close()


def fetch_hybrid_search_chunks_and_scores(question):
    """
    Retrieves the most relevant text chunks for a given question using Hybrid search.
    ...
    try:
        ...
        dense_search_param = {
            "metric_type": 'L2',
            "params": {"ef": NUM_CHUNKS_TO_SIMILARITY_SEARCH + 100},
        }
        sparse_search_param = {
            "metric_type": "BM25"
        }
        vectorstore = Milvus(
            ...
            search_params=[dense_search_param, sparse_search_param],
            consistency_level="Strong",
        )
        retrieved_docs = vectorstore.similarity_search_with_score(
            query=question, k=NUM_CHUNKS_RETRIEVED, fetch_k=NUM_CHUNKS_TO_SIMILARITY_SEARCH, ranker_type="weighted", ranker_params={"weights": [DENSE_VECTOR_WEIGHT, SPARSE_VECTOR_WEIGHT]})
        ...
        print(
            f"An error occurred while fetching hybrid search chunks and scores for question: {question}: {e}")
        return None


def save_chunks_to_stringio():
    """
    Fetches questions without citations, retrieves relevant chunks using hybrid search,
    ...
    for idx_q, question in enumerate(questions, start=1):
        ...
        chunks = fetch_hybrid_search_chunks_and_scores(question)
        if not chunks:
        ...
        for idx_c, chunk in enumerate(chunks, start=1):
            ...
            file_content.write(f"Score: {chunk['score']}\n")
